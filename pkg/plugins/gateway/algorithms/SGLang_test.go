...
func Test_LPRadixCacheE2E(t *testing.T) {
	// cache := NewSGLangTreeCache(2) // assuming 2 GPUs
	pods := []*v1.Pod{
		{ObjectMeta: metav1.ObjectMeta{Name: "p1"}},
		{ObjectMeta: metav1.ObjectMeta{Name: "p2"}},
	}

	// extract pods.name into []string
	pods_name := []string{}
	for _, pod := range pods {
		pods_name = append(pods_name, pod.Name)
	}

	SGLang_router, _ := NewSGLangRouter(pods_name)

	SGLang_router.Route(context.TODO(), pods[0], "m1", "")
	...
	// 	matchedTokens)
	// assert.Equal(t, 0, len(tokens)-len(matchedTokens))
...
func Test_SGLang(t *testing.T) {
	...
	// inputText1 := "hello"
	// tokens1, _ := utils.TokenizeInputText(inputText1)

	// inputText2 := "apple"
	// tokens2, _ := utils.TokenizeInputText(inputText2)

	// inputText3 := "banana"
	// tokens3, _ := utils.TokenizeInputText(inputText3)

	// // log the tokens1, tokens2, tokens3
	// log.Println(tokens1)
	// log.Println(tokens2)
	// log.Println(tokens3)
	// 2025/03/12 22:38:46 [15339]
	// 2025/03/12 22:38:46 [23182]
	// 2025/03/12 22:38:46 [88847]
